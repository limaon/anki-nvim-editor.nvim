-- Commands for anki-nvim-editor

local M = {}
local anki_connect = require("anki-nvim-editor.anki_connect")
local ui = require("anki-nvim-editor.ui")
local buffers = require("anki-nvim-editor.buffers")
...
function M.edit_template(state)
  require("anki-nvim-editor").notify("Fetching model list...", "info")

  anki_connect.get_model_names(state, function(models, error)
    if error then
      require("anki-nvim-editor").notify("Error fetching models: " .. error, "error")
      return
    end

    if not models or #models == 0 then
      require("anki-nvim-editor").notify("No models found", "warn")
      return
    end
    ...
    ui.select_template(state, models, function(model, card, side)
      if not model or not card or not side then
        require("anki-nvim-editor").notify("Selection cancelled", "info")
        return
      end
      ...
      anki_connect.get_model_templates(state, model, function(templates, error1)
        if error1 then
          require("anki-nvim-editor").notify("Error fetching templates: " .. error1, "error")
          return
        end

        anki_connect.get_model_styling(state, model, function(css, error2)
          if error2 then
            require("anki-nvim-editor").notify("Error fetching styling: " .. error2, "error")
            return
          end
...
function M.ping(state)
  require("anki-nvim-editor").notify("Testing Anki-Connect connection...", "info")
